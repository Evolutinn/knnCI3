...
class Data_uji extends Admin_Controller {
  ...
  public function import()
  {
      ...
      error_reporting(0);  // Nonaktifkan semua error
      ...
      if ($this->upload->do_upload("document_file")) {
          try {
              ...
              $loadexcel = $excelreader->load('upload/datatestingexcel/' . $filename); // Load file Excel yang diupload
              ...
              foreach ($sheet as $row) {
                  // Proses hanya baris data (bukan header)
                  if ($numrow > 1 && !empty($row['A'])) {
                      // Proses data untuk setiap peserta
                      $data_uji = array();
                      ...
                      // Tambahkan user baru
                      $identitas = time();
                      $data_user['user_username'] = uniqid('user_');
                      $data_user['user_password'] = password_hash($identitas, PASSWORD_DEFAULT);
              ...
              // Tangani error dengan menampilkan pesan error yang lebih informatif
              log_message('error', 'Error during file upload or processing: ' . $e->getMessage());
              $this->session->set_flashdata('info', array(
                  'from' => 0,
